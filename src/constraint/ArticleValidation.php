...
class ArticleValidation extends Validation
{
    ...
    private function checkphoto($name, $value)
    {
        if($this->constraint->notBlank($name, $value)) {
            return $this->constraint->notBlank('photo', $value);
        }
        if($this->constraint->minLength($name, $value, 255)) {
            return $this->constraint->minLength('photo', $value, 255);
        }
